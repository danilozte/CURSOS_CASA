...
const unsigned long INTERVALO_MEDIDA = 5000UL; // SON LAS CONSTANTE DE TIEMPO DADAS EN TIPO UNSIGNED LONG
const unsigned long INTERVALO_LED = 5000UL; // INTERVALO DE TIEMPO PARA CADA ACCION
const int pinLED = 11;
 
unsigned long evento_led;
unsigned long evento_medida;
...
void loop() {
  ...
  if( actual > evento_led ){
    ...
    evento_led += INTERVALO_LED; // SE LE CARGA 5000UL A EVENTO_LED CADA VEZ QUE DESBORDE EL VALOR DEL INTERVALO   
  } 
  else {
  Serial.print("aun no pasan 5 segundos para cambiar el led");
  }
 
  if( actual == INTERVALO_MEDIDA ){
    sensor();
    //evento_medida += INTERVALO_MEDIDA; //IGUAL PARA ESTA FUNCION SOLO QUE VAMOS LEEMOS EL VALOR ANALO Y MOSTRAMOS
  }
 else{
  Serial.println("aun no pasan 5 segundos para medir");
  }
